...
- **Register** them with Remnawave Panel via the official REST API.
...
- **Rotate** IPs on three triggers:
  ...
  3. Manual via REST API or `remna-ctl rotate <id>`.
...
## Not implemented in this MVP (contributions welcome)

- Graceful client drain before deleting old node.
- Multi-region pool balancing.
- PostgreSQL backend (SQLAlchemy is already configured, just swap `DATABASE_URL`).
- Webhook notifications (Telegram/Discord) for rotation events.
